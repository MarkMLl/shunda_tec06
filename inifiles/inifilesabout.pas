(* Lazarus+FPC 0.9.24+2.2.4 to 2.2.4+3.2.2. On Linux for ARM, PPC, SPARC, x86.  *)
...
type
  SoundFunc= function(): boolean;

(* Register external sound generator procedures, typically implemented by ALSA.
*)
procedure RegisterSound(wave, midi: SoundFunc);

(* This is the RTL's Format(), with the following enhancements:

    %A    ARPA (RFC 2822) format local date and time.
    %0A   ARPA (RFC 2822) format uncorrected date and time.
    ...
    %S    Expands the final parameter as a +ve decimal message serial number.
  ...
  Note that case-sensitive %U is expanded before FPC's case-insensitive Format()
  attempts to expand %u.

  Note also the distinction between the two special cases %0I and %0U, which are
  not standard Format() or printf() patterns, as compared with %0:i and %0:u
  which are. The final two optional parameters are applicable only to ISO-format
  dates.
*)
function FormatEx(const str: string; values: array of const;
                                        format: TIsoDateTimeFormat;
                                        fractionDigits: int64= 0): string;
...
{ define DEBUG_DEFINES }

{$ifdef DEBUG_DEFINES  }
  {$ifdef LCL }
    {$warning In IniFilesAbout, LCL set }
  {$else             }
    {$warning In IniFilesAbout, LCL not set }
  {$endif LCL }
  {$ifdef HAS_SCREEN }
    {$warning In IniFilesAbout, HAS_SCREEN set }
  {$else             }
    {$warning In IniFilesAbout, HAS_SCREEN not set }
  {$endif HAS_SCREEN }
  {$ifdef HAS_LCLVERSION }
    {$warning In IniFilesAbout, HAS_LCLVERSION set }
  {$else             }
    {$warning In IniFilesAbout, HAS_LCLVERSION not set }
  {$endif HAS_LCLVERSION }
  {$ifdef HAS_FCLRES }
    {$warning In IniFilesAbout, HAS_FCLRES set }
  {$else             }
    {$warning In IniFilesAbout, HAS_FCLRES not set }
  {$endif HAS_FCLRES }
  {$ifdef SVN }
    {$warning In IniFilesAbout, SVN set }
  {$else             }
    {$warning In IniFilesAbout, SVN not set }
  {$endif SVN }
  {$ifdef HAS_SVN }
    {$warning In IniFilesAbout, HAS_SVN set }
  {$else             }
    {$warning In IniFilesAbout, HAS_SVN not set }
  {$endif HAS_SVN }
  {$ifdef HAS_SUBVERSION }
    {$warning In IniFilesAbout, HAS_SUBVERSION set }
  {$else             }
    {$warning In IniFilesAbout, HAS_SUBVERSION not set }
  {$endif HAS_SUBVERSION }
  {$ifdef FPC_SVN }
    {$warning In IniFilesAbout, FPC_SVN set }
  {$else             }
    {$warning In IniFilesAbout, FPC_SVN not set }
  {$endif FPC_SVN }
  {$ifdef HAS_FPC_SVN }
    {$warning In IniFilesAbout, HAS_FPC_SVN set }
  {$else             }
    {$warning In IniFilesAbout, HAS_FPC_SVN not set }
  {$endif HAS_FPC_SVN }
  {$ifdef HAS_FPC_SUBVERSION }
    {$warning In IniFilesAbout, HAS_FPC_SUBVERSION set }
  {$else             }
    {$warning In IniFilesAbout, HAS_FPC_SUBVERSION not set }
  {$endif HAS_FPC_SUBVERSION }
{$endif DEBUG_DEFINES  }

USES (*$ifdef HAS_SCREEN *) Forms, Graphics, (*$endif HAS_SCREEN *) DateUtils
        (*$IFDEF HAS_LCLVERSION *) , LCLVersion (*$ENDIF HAS_LCLVERSION *)
        (*$IFDEF HAS_FCLRES *) , resource, versiontypes, versionresource (*$ENDIF HAS_FCLRES *)
        , exeinfo
        {$ifdef UNIX} , Linux, BaseUnix, UnixUtil
                {$ifdef LCLGTK } , gtk {$endif LCLGTK1 }
                {$ifdef LCLGTK2 } , gtk2 {$endif LCLGTK2 }
                {$ifdef LCLQTX } , qtint {$endif LCLQTX }
        {$endif UNIX } ;
...
CONST   lcl_major = 0;
  ...
  (*$ENDIF                *)
        ...
        lcl_fullversion2 = ((lcl_major *  1000 + lcl_minor) * 1000 + lcl_release) * 1000 + lcl_patch;
(*$ENDIF LCL *)
...
CONST   manifestDate= (*$I %DATE% *) ;
        ...
        isoDTT= 'yyyy-mm-dd"T"hh":"nn":"ss';
        isoDUT= 'yyyy-mm-dd"_"hh":"nn":"ss';
        arpaDT= 'ddd, d mmm yyyy hh:nn:ss'; (* Four-digit year is RFC 2822, not 822 *)

VAR     cachedResourceVersion: QWORD= uncached;
        cachedBuildId: QWORD= uncached;

(* Depending on the widget set, the version information might be constants or   *)
(* variables and while in some cases it might be possible to initialise them    *)
(* here in others they have to be fetched after the underlying libraries have   *)
(* been fully initialised.                                                      *)

{$ifdef LCLGTK2 }
VAR     ws_major_version: cuint; external gtklib name 'gtk_major_version';
...
{$else          }
VAR     ws_major_version: cuint= 0;
        ws_minor_version: cuint= 0;
        ws_micro_version: cuint= 0;
{$endif LCLGTK2 }
...
BEGIN
  ...
  RESULT:= 0;
  cachedBuildId := 0;
  IF OpenExeFile(binFile{%H- 5057 }, ParamStr(0)) THEN TRY
    IF NOT FindExeSection(binFile, '.note.gnu.build-id', secOfs{%H- 5057 }, secLen{%H- 5057 }) THEN
...
//TODO : Add note about using readelf/objcopy, and allow for more than one item in section.
// readelf -n will display the notes section(s). It might be possible to use
// objcopy to extend the build-id field from the normal 128 or 160 bits (16
// or 20 bytes), in which case if it looks oversize consider that its length
// might have been doubled to contain e.g. the difference between the current
// build-id and the one that was calculated for v0.0 of a program so that a
// default configuration file can be checked to match.
...
(* ../../../../local/share.lazarus/lazarus-stable/ide/version.inc               *)
...
(*$ENDIF       *)
(*$IFDEF LCLGTK *)
	  widV= 'Linked with the GTK vx.x.x widget set';
  (*$DEFINE WIDGETSET *)
(*$ENDIF       *)
(*$IFDEF LCLGTK2 *)
	  widV= 'Linked with the GTK vx.x.x widget set';
...
(*$ENDIF       *)
(*$IFDEF LCLQTX *)
	  widV= 'Linked with the Qt vx.x.x widget set';
  (*$DEFINE WIDGETSET *)
(*$ENDIF       *)
(*$IFDEF LCLFPGUI *)
...
(*$ENDIF	*)
  ...
  (* At runtime, report the actual widgetset version.
  *)
  function wsVer(const str: string): string;

  begin
    result := str;
  (*$IFDEF LCLGTK *)
    ws_major_version := gtk_major_version;
    ws_minor_version := gtk_minor_version;
    ws_micro_version := gtk_micro_version;
  (*$ENDIF LCLGTK *)
  (*$IFDEF LCLGTK3 *)
    ws_major_version := gtk_get_major_version();
    ws_minor_version := gtk_get_minor_version();
    ws_micro_version := gtk_get_micro_version();
  (*$ENDIF LCLGTK3 *)
  {$ifdef LCLQTX }
    ws_major_version := QtVersionMajor;
    ws_minor_version := QtVersionMinor;
    ws_micro_version := QtVersionMicro;
  {$endif LCLQTX }
    result := StringReplace(str, 'vx.x.x', 'v' + IntToStr(ws_major_version) + '.' +
                                                IntToStr(ws_minor_version) + '.' +
                                                IntToStr(ws_micro_version), [rfReplaceAll]);
    result := StringReplace(result , ' vx.x.x', '', [rfReplaceAll])
  end { wsVer } ;
...
(*$IFDEF LCL *)
  IF (RevisionStr = 'exported') or (Pos('Unversioned', RevisionStr) = 1) or (RevisionStr = '') THEN
    RESULT:= RESULT + sanitize(svn(about1)) + about2b + about3 + fpcSvn(about4) + about5 + wsVer(about6)
  ELSE
    RESULT:= RESULT + sanitize(svn(about1)) + about2a + about3 + fpcSvn(about4) + about5 + wsVer(about6);
(*$ELSE      *)
...
var
  waveFunc: SoundFunc= nil;
  midiFunc: SoundFunc= nil;
...
(* Interpret any number of \a sequences as a single beep via the PCM subsystem.
*)
function waveBeep(const s: string): string;

begin
  result := s;
  if Assigned(waveFunc) then begin
    if waveFunc() then
...
(* Interpret any number of \A sequences as a single beep via the MIDI subsystem.
*)
function midiBeep(const s: string): string;

begin
  if Assigned(midiFunc) then
    if midiFunc() then
      result := StringReplace(s, '\A', '', [rfReplaceAll])
    else
      result := StringReplace(s, '\A', '\a', [rfReplaceAll])
  else
    result := StringReplace(s, '\A', '\a', [rfReplaceAll])
end { midiBeep } ;
...
var     scratch, timeStr: string;
  ...
  (* Return the position of the first pattern \9T or -1, and parse the digit.
  *)
  function nt(const s: string; out n: integer): integer;
  ...
  begin
    ...
    for i := 1 to Length(s) - 2 do
      if (s[i] = '\') and (s[i + 1] in ['0'..'9']) and (s[i + 2] = 'T') then begin
        n := StrToInt(s[i + 1]);
        if n = 0 then
          n := 10;
        exit(i)
  ...
  (* Return the position of the first pattern \9.9T or -1, and parse the digits.
  *)
  function nxt(const s: string; out n, x: integer): integer;
  ...
  begin
    ...
    for i := 1 to Length(s) - 4 do
      if (s[i] = '\') and (s[i + 1] in ['0'..'9']) and (s[i + 2] = '.') and
                                (s[i + 3] in ['0'..'9']) and (s[i + 4] = 'T') then begin
        n := StrToInt(s[i + 1]);
        if n = 0 then
          n := 10;
        x := StrToInt(s[i + 3]);
...
begin
  scratch := StringReplace(str, '\%', percent, [rfReplaceAll]);
...
(* This is the ARPA date and time format prescribed by RFC 2822, i.e. similar   *)
(* to RFC 822 but with a four-digit year. Local time is indicated using the     *)
(* -0000 notation, i.e. it's not UTC +- some offset.                            *)

  if Pos('%A', scratch) > 0 then begin
    timeStr := FormatDateTime(arpaDT, Now()) + ' -0000';
    scratch := StringReplace(scratch, '%A', timeStr, [rfReplaceAll])
  end;
  if Pos('%0A', scratch) > 0 then begin
    timeStr := FormatDateTime(arpaDT, UTC_Now()) + ' +0000';
    scratch := StringReplace(scratch, '%0A', timeStr, [rfReplaceAll])
  end;

(* Tentatively repurpose the final parameter as e.g. a message serial number.   *)
(* It's the user's responsibility to not attempt to use %I and %S in the same   *)
(* error message etc.                                                           *)

{$push }
{$rangechecks off    }
{$overflowchecks off }
  scratch := StringReplace(scratch, '%S', IntToStr(qword(fractionDigits)), [rfReplaceAll]);
{$pop  }
  scratch := StringReplace(scratch, percent, '%', [rfReplaceAll]);
  result := Sysutils.Format(scratch, values);   (* Anything else                *)

(* Backslash-escaped characters or sequences have to be processed after         *)
(* Format() has expanded all printable output. Note that embedded \n etc. is    *)
(* probably incompatible with the \T expansions.                                *)

  result := StringReplace(result, '\\', backslash, [rfReplaceAll]);
  if GetEnvironmentVariable('DISPLAY') = ':0' then
    result := midiBeep(result)          (* Will delete or replace \A            *)
  else
    result := StringReplace(result, '\A', '\a', [rfReplaceAll]);
  if GetEnvironmentVariable('DISPLAY') = ':0' then
    result := waveBeep(result);         (* Will delete \a                       *)
  result := StringReplace(result, '\a', #$07, [rfReplaceAll]);
  result := StringReplace(result, '\t', #$09, [rfReplaceAll]);
  result := StringReplace(result, '\n', #$0a, [rfReplaceAll]);
  result := StringReplace(result, '\N', LineEnding, [rfReplaceAll]);
  result := StringReplace(result, '\r', #$0d, [rfReplaceAll]);

(* Assume that the result is more or less in its final form. Expand any \T etc. *)
(* escapes, noting that this is probably incompatible with any line breaks.     *)

// TODO : Scan from right to find line-breaks, "real" tabs etc.

  t := Pos('\T', result);
  while t > 0 do begin
    scratch := Copy(result, 1, t - 1);
    Delete(result, 1, t + 1);
{$if declared(PadRight) }
    t := Length(scratch) + (8 - (Length(scratch) mod 8));
    scratch := PadRight(scratch, t);
{$else                  }
    while (Length(scratch) - 1) mod 8 <> 7 do
      scratch += ' ';
{$endif                 }
  ...
  while t > 0 do begin
    scratch := Copy(result, 1, t - 1);
    Delete(result, 1, t + 2);
    scratch += ' ';                     (* Then pad to tab position n           *)
{$if declared(PadRight) }
    t := n * 8;
    scratch := PadRight(scratch, t);
{$else                  }
    while Length(scratch) < n * 8 do
      scratch += ' ';
{$endif                 }
    result := scratch + result;
    t := nt(result, n)
...
{$else                  }
    while Length(scratch) < n * 8 do
      scratch += ' ';
    if (x > 1) and (x mod 8 = 1) then   (* Finally append an extra x spaces     *)
      x -= 1;
    while x > 0 do begin
      scratch += ' ';
      x -= 1
...
(* Test the various \T expansions.
*)
procedure testTabs;

begin
  WriteLn('         1         2         3         4         5         6         7         8');
  WriteLn('123456789012345678901234567890123456789012345678901234567890123456789012345678901');
  WriteLn(FormatEx('\T9\T7\T5\T3\T1\T9\T7\T5\T3\T1', []));
  WriteLn(FormatEx(' \T9\T7\T5\T3\T1\T9\T7\T5\T3\T1', []));
  WriteLn(FormatEx('\1T|\2T|\3T|\4T|\5T|\6T|\7T|\8T|\9T|\0T|', []));
  WriteLn(FormatEx('\1.1T0\2.3T0\3.5T0\4.7T0\5.9T0\7.3T0\8.5T0\9.7T0', []));
  WriteLn(FormatEx('\1.1T0\2.3T0\3.5T0\4.7T0\6.1T0\7.3T0\8.5T0\9.7T0', []));
  WriteLn(FormatEx('\9.5TEOL', []))     (* E should be in column 78             *)
end { testTabs } ;
...
initialization
(*$IFDEF LCL *)
  if lcl_fullversion = lcl_fullversion2 then ;  (* Avoid compiler "unused" hint *)
(*$ENDIF LCL *)
{$ifopt C+ Predicated on assertions generating code }
//  testIso;
//  testTabs
